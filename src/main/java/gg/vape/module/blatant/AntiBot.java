...
extends Mod {
    ...
    private final BooleanValue teamsByArmorColor;
    ...
    public boolean isArmorColorTeamFilteringEnabled() {
        return this.isEnabled() && this.teamsByArmorColor.getEffectiveValue() != false;
    }
    ...
    public boolean isTeammate(@Nullable EntityPlayerSP localPlayer, Entity entity) {
        ...
        if (!this.isColorTeamFilteringEnabled() && !this.isServerTeamFilteringEnabled() && !this.isArmorColorTeamFilteringEnabled()) {
            return false;
        }
        ...
        if (entity.isInstance(MappedClasses.lG)) {
            ...
            if (this.isArmorColorTeamFilteringEnabled()) {
                int localColor = this.getLeatherChestplateColor(resolvedLocalPlayer);
                int otherColor = this.getLeatherChestplateColor(otherPlayer);
                if (localColor != -1 && otherColor != -1 && localColor == otherColor) {
                    return true;
                }
            }
    ...
    private int getLeatherChestplateColor(EntityPlayer entityPlayer) {
        InventoryPlayer inv = entityPlayer.V$src$Lgg_vape_wrapper_impl_InventoryPlayer_$erqak6();
        if (inv.isNull()) {
            return -1;
        }
        Object[] armorItems = inv.i();
        if (armorItems == null || armorItems.length <= 2 || armorItems[2] == null) {
            return -1;
        }
        ItemStack chestplate = armorItems[2] instanceof ItemStack ? (ItemStack) armorItems[2] : new ItemStack(armorItems[2]);
        if (chestplate.isNull()) {
            return -1;
        }
        Item chestplateItem = chestplate.getItem();
        if (chestplateItem.isNull() || !chestplateItem.isInstance(MappedClasses.qU)) {
            return -1;
        }
        return MItemArmor.w(Vape.INSTANCE.getMappings().DZ, chestplateItem.getObject(), chestplate.getObject());
    }
    ...
    public AntiBot() {
        ...
        this.teamsByColor = BooleanValue.create(this, "Name color", false, "Ignore players with the selected name color\n\u00a7cThis is not guaranteed to be accurate - team colors depend on the server implementation");
        this.teamsByArmorColor = BooleanValue.create(this, "Armor color", false, "Ignore players wearing the same leather chestplate color\n\u00a7cCompares the color of leather chestplates to determine teams");
        ...
        this.addValue(this.teamsByServer, this.teamsByColor, this.teamsByArmorColor, this.recolorVisuals);
