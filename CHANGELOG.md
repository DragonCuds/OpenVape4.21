...
## v4.21.36 Expansion and Restoration (2026-09-10)

**模块开关音效（Toggle Sound）+ SilentAura AI 模式治理 + 版本号更新**

- **新增模块开关音效**：模块开启/关闭时播放 `enable`/`disable` 提示音（随全局通知音量与静音设置）
- **同按键多模块合并播放**：一个按键绑定多个模块时，一次按键只播放一次音效（开启优先），
  不再每个模块各响一次；通过 `MouseInputState`/`KeyboardInputState` 事件批次追踪聚合实现
- **切换配置档同时播放**：应用配置档时强制逐个出声，所有被开关的模块音效同时播放

- **SilentAura AI 模式启动自动关闭**：启动注入（配置档状态应用完成）时检测到 SilentAura
  为 AI 旋转模式且已启用 → 自动关闭一次并弹出通知；仅启动时执行一次，不修改配置本身
- **SilentAura AI 模式隐藏设置**：AI 旋转模式下 `Aim speed` 与 `Target area` 不适用
  （模型直接输出角度），自动隐藏；切回 PID 模式恢复显示

- 若遇崩溃请反馈日志

## v4.21.28 Expansion and Restoration (2026-08-25)
